 ...
 *
 * @see lidis.Reservation
 */
@Scope("singleton")
@Repository("ReservationDAO")
public class ReservationDAO extends HibernateDaoImpl<Reservation, Integer>
    implements IReservationDAO {
    private static final Logger log = LoggerFactory.getLogger(ReservationDAO.class);
    @Resource
    private SessionFactory sessionFactory;

    public static IReservationDAO getFromApplicationContext(
        ApplicationContext ctx) {
        return (IReservationDAO) ctx.getBean("ReservationDAO");
    }
}
